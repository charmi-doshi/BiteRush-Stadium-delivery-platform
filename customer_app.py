...
# # ── Session state ──────────────────────────────────────────────────────────────
# for k, v in {"cart": {}, "order_placed": False, "last_orders": []}.items():
#     if k not in st.session_state:
#         st.session_state[k] = v

# # ── Helpers ────────────────────────────────────────────────────────────────────
# def build_es_doc(floor, row, seat, item_id, payment) -> dict:
#     item = MENU_BY_ID[item_id]
#     oid  = "ORD" + uuid.uuid4().hex[:6].upper()
#     source = {
#         "order_id":      oid,
#         "vendor_id":     VENDOR_ID,
#         "item_ordered":  item["id"],
...
#     print("\n" + "="*52)
#     print("  FRANKY VADAPAV — NEW ORDER(S)")
#     print("="*52)
#     for doc in orders:
...
#     """
#     Index a single order doc into Elasticsearch using API key auth.
#     PUT {ES_BASE_URL}/{index}/_doc/{order_id}
...
#             url,
#             headers=headers,
#             data=json.dumps(doc["_source"]),
#             timeout=8,
#         )
#         if resp.status_code in (200, 201):
#             result = resp.json().get("result", "ok")   # "created" or "updated"
#             return True, result
#         else:
...
#     orders   = st.session_state.last_orders
#     ids      = [o["_id"] for o in orders]
#     seat_str = f"{orders[0]['_source']['floor_level']} · Row {orders[0]['_source']['row_number']} · Seat {orders[0]['_source']['seat_number']}"
#     max_prep = max(o["_source"]["prep_time_sec"] for o in orders)
#     eta      = max(1, round((max_prep + 60) / 60))

#     id_divs = "".join(f'<div class="oid">{oid}</div>' for oid in ids)
#     st.markdown(f"""
...
#       {id_divs}
#       <p style="margin-top:10px;">Estimated wait: <strong>~{eta} min</strong></p>
#     </div>
#     """, unsafe_allow_html=True)
...
#              type="primary", disabled=not can_order):
...
#             doc = build_es_doc(floor, row, seat, item_id, payment)
#             docs.append(doc)

#         print_orders(docs)

#         # ── Push each doc to Elasticsearch ────────────────────────────────
#         es_errors = []
...
#                 else:
#                     es_errors.append(f"{doc['_id']}: {msg}")
#                     print(f"  ES error  {doc['_id']}: {msg}")

#         if es_errors:
#             st.error("Order saved locally but Elasticsearch indexing failed:\n" +
#                      "\n".join(es_errors))

#         st.session_state.last_orders  = docs
...
def build_es_doc(floor, row, seat, cart: dict, payment) -> dict:
    ...
    # item_ordered → plain array of ids, one per unique item (no qty expansion)
    item_ordered = list(cart.keys())   # ["fries", "beer"]

    # derive food_type and prep_time_sec from the item with the longest prep
...
if st.session_state.order_placed and st.session_state.last_order:
    ...
    eta = max(1, round((src["prep_time_sec"] + 60) / 60))
